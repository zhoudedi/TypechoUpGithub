 ...
 * 附件上传Github仓库插件
 *
 * @package TypechoUpGithub
 * @author 小周
 * @link https://github.com/zhoudedi/TypechoUpGithub
 * @version 1.0.0
 * @dependence 1.0-*
 *
 */

class TypechoUpGithub_Plugin implements Typecho_Plugin_Interface
{
    //上传文件目录
    ...
    {
        Typecho_Plugin::factory('Widget_Upload')->uploadHandle = array('TypechoUpGithub_Plugin', 'uploadHandle');
        Typecho_Plugin::factory('Widget_Upload')->modifyHandle = array('TypechoUpGithub_Plugin', 'modifyHandle');
        Typecho_Plugin::factory('Widget_Upload')->deleteHandle = array('TypechoUpGithub_Plugin', 'deleteHandle');
        Typecho_Plugin::factory('Widget_Upload')->attachmentHandle = array('TypechoUpGithub_Plugin', 'attachmentHandle');
        Typecho_Plugin::factory('Widget_Upload')->attachmentDataHandle = array('TypechoUpGithub_Plugin', 'attachmentDataHandle');
        return _t('TypechoUpGithub插件已激活，请前往设置');
    }
    ...
    {
        return _t('TypechoUpGithub插件已禁用');
    }
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
        echo '
                ...
                <p id="TypechoUpGithub-check-update" class="notice">正在检查TypechoUpGithub插件最新状态...</p>
                <script>
                    window.onload = function()
                    {
                        ...
                        var notice = "正在检查更新...";
                        $.ajax({
                            url: "https://api.github.com/repos/zhoudedi/TypechoUpGithub/releases",
                            async: true,
                            type: "GET",
                            success: function (data) {
                                ...
                                if(newest == null){
                                    ...
                                    notice = "插件需要更新，当前版本：v" + now + "，最新版本：v" + newest + "。<a href=\'https://github.com/zhoudedi/TypechoUpGithub\'>点击这里</a>获取最新版本。";
                                }
                                $(\'#TypechoUpGithub-check-update\').html(notice);
                            },
        ...
        <ol>
        <li>本插件用于将文章附件(如图片)上传至您的(公开的)Github的仓库中，并使用jsDelivr访问仓库文件达到优化文件访问速度的目的。了解jsDelivr应用于博客中的优势，您可以<a href='https://www.jsdelivr.com/?docs=gh' target='_blank'>点击这里</a>。<br></li>
        <li>项目地址：<a href='https://github.com/zhoudedi/TypechoUpGithub' target='_blank'>https://github.com/zhoudedi/TypechoUpGithub</a><br></li>
        <li>插件使用说明与教程：<a href='https://zhuanlan.zhihu.com/p/550294543' target='_blank'>https://zhuanlan.zhihu.com/p/550294543</a><br></li>
        ...
        $commit_name = new Typecho_Widget_Helper_Form_Element_Text('commitName', NULL, 'TypechoUpGithub', _t('提交文件者名称'), _t('提交Commit的提交者名称，留空则为仓库所属者。'));
        $commit_email = new Typecho_Widget_Helper_Form_Element_Text('commitEmail', NULL, 'TypechoUpGithub@typecho.com', _t('提交文件者邮箱'), _t('提交Commit的提交者邮箱，留空则为仓库所属者。'));
        $form->addInput($desc1);
    ...
    public static function uploadHandle($file)
    {
        ...
        if ($http_code != 201) {
            ...
            self::writeErrorLog($path_relatively, "[Github 仓库上传失败!!! ]" ." | ". $http_code .  " | " . $output['message']);
        }else if ($http_code = 201) {
            $output = json_decode($output,true);
            self::writeErrorLog($path_relatively, "[Github 仓库上传成功! ]"." | " . $http_code ." | ". $output['message']);
            self::uploadLog($path_relatively, "[Github 仓库上传成功! ]" ." | ". $http_code . " | " . $output['message']);
        }
        ...
        if ($options->ifSave == 'save') {
            if (!is_dir($fileDir)) {
                if (self::makeUploadDir($fileDir)) {
                    ...
                    self::writeErrorLog($path_relatively, "[本地]目录、文件创建成功! ". "\n");
                } else {
                    //文件写入失败，写入错误日志
                    self::writeErrorLog($path_relatively, "[本地]目录、文件创建失败!!! 请给插件目录、子目录777权限". "\n");
                }
            } else {
                file_put_contents($path, $fileContent);
                $filename = __TYPECHO_ROOT_DIR__ . $path;
                self::writeErrorLog($filename, "[本地]文件创建成功! ". "\n");
            }
        }
    ...
    public static function modifyHandle($content, $file)
    {
        ...
        if ($http_code != 200) {
            $output = json_decode($output,true);
            self::writeErrorLog($github_path, "[Github 仓库修改失败!!! ]" ." | ". $http_code ." | " . $output['message']);
        }else if($http_code = 200){
            $output = json_decode($output,true);
            self::writeErrorLog($github_path, "[Github 仓库修改成功! ]" ." | ". $http_code ." | " . $output['message']);
        } 
        ...
        if ($options->ifSave == 'save') {
            ...
            self::writeErrorLog($filename, "[本地]文件修改成功! ". "\n");
        }
    ...
    public static function deleteHandle(array $content)
    {
        ...
        if ($http_code != 200) {
            $output = json_decode($output,true);
            self::writeErrorLog($github_path, "[Github 仓库删除失败!!! ]" ." | ". $http_code ." | " . $output['message']);
        }else if($http_code = 200) {
            $output = json_decode($output,true);
            self::writeErrorLog($github_path, "[Github 仓库删除成功! ]" ." | ". $http_code ." | " . $output['message']);
        }
        //删除本地文件
        if ($options->ifSave == 'save' && file_exists($filename) == true) {
            unlink($filename);
            self::writeErrorLog($filename, "[本地]文件删除成功! ". "\n");
        }else{
            self::writeErrorLog($filename, "[本地]文件删除失败!!! ". "\n");
        }
     ...
     * 上传文件生成链接log/img.txt
     */
    private static function uploadLog($path, $content)
    {
        #$date = date('[Y/m/d H:i:s]', time());
        #$text = $date . " " . $path . " " . $content . "\n";
        $options = Typecho_Widget::widget('Widget_Options')->plugin('TypechoUpGithub');
